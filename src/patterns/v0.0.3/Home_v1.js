...
import withHeaders from '../../common/hoc/withHeaders';
...
const CashCard = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const myWallet = await controller.cashCadInitData(props.headers);
      console.log('effect, didmount', myWallet);
      setData(myWallet && myWallet.results);
    }
    fetchData();
    return () => {
  ...
  const handleClick = async () => {
    ...
    const isLogin = await controller.isLogin(props.headers);

    return isLogin ? history.push('/cash/history') : history.push('/login');
  };
